...
open class MovieManager: NetworkManager {
    ...
    open func load(
        ...
        self.manager.request(APIManager.getBaseUrl() + PopcornMovies.movies + "/\(page)", parameters: params).validate().responseJSON { response in
    ...
    open func getInfo(_ imdbId: String, completion: @escaping (Movie?, NSError?) -> Void) {
        self.manager.request(APIManager.getBaseUrl() + PopcornMovies.movie + "/\(imdbId)").validate().responseJSON { response in
